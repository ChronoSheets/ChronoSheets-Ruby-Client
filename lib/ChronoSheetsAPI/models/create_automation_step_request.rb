...
module ChronoSheetsAPI
  class CreateAutomationStepRequest
    attr_accessor :geofencing_id

    attr_accessor :nfc_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChronoSheetsAPI::CreateAutomationStepRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChronoSheetsAPI::CreateAutomationStepRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'geofencing_id')
        self.geofencing_id = attributes[:'geofencing_id']
      end

      if attributes.key?(:'nfc_id')
        self.nfc_id = attributes[:'nfc_id']
      end
      ...
      if attributes.key?(:'client_time')
        self.client_time = attributes[:'client_time']
      end
    end
    ...
    def hash
      [geofencing_id, nfc_id, automation_action_type, latitude, longitude, client_time].hash
    end
